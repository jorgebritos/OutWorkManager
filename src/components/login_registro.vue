    ...
    <q-card class="fondo_azul col-xs-12 col-md-6 col-lg-5 row justify-center items-center">
            <q-card class="my-card col-sm-7 col-xs-10 col-md-11 col-lg-10 row">
                ...
                <q-card-section class="col-12 flex justify-center">
                    ...
                    <q-btn :label="pagina.texto" type="submit" class="boton_enviar" @click="enviar_registro"/>
                </q-card-section>
            </q-card>
        </q-card>
</template>
<script setup>
    ...
    const cambiar_estado = (nombre,estado) => {
        ...
        if (nombre === "password" || nombre === "repetir_password") {
            entrada.estado = !entrada.estado;
            entrada.tipo = entrada.tipo === "password" ? "text" : "password";
        }
    }

    const enviar_registro = () => {
        router.push(`${pagina.value.pagina}`)
    }
...
<style scoped>
  ...
  transform: scale(1.1);
 color: rgb(8, 93,112);
 cursor: url('src/assets/llave16.png'), auto;
